...
/*
	var semaforoTacchi = true;
    var indexT = 0;

    $('.mk-floating-vertical').click(function () {
        var contTacchi = $('.ricambi_tacchi');
        ++indexT;
        if (semaforoTacchi) {
            $('#tacco1').hide();
        }
        contTacchi.find("#t" + indexT).animate({ opacity: '0' }, 100, function () {
            contTacchi.find("#t" + (indexT + 1)).animate({ opacity: '1' }, 200);
        });
...
*/
      ...
      $('.mk-header-inner').append('<div class="logo_pomo"><a href="http://www.pomodoro.com" target="_blank"><img src="/wp-content/themes/conspiracy/images/pomodoro.png" alt="Pomodoro srl" class="logo_pomodoro"></a></div>');	
		
	$(".logo_pomodoro").hover(function () {  			
			$('.logo_pomodoro').attr("src","/wp-content/themes/conspiracy/images/pomodoro-h.png");
		},function () {	
			$('.logo_pomodoro').attr("src","/wp-content/themes/conspiracy/images/pomodoro.png");
		}
	);	             
           
           
    $('#mk-filter-portfolio ul li a').click(function (e) {	    
	      var $this;
	      $this = $(this);			
	...
	$("input[name=informativa]").prop('checked', false);
	...
	$(".privacy #checkbox").bind("click", function (e) {
		...
		if($("input[name=informativa]").is(":checked")) {
				...
				// check
				$("input[name=informativa]").prop('checked', true);
...
/* Posizionare plus in home */
/* -------------------------------------------------------------------- */
function position_plus() {
    if (jQuery('#tacco1').length !=0) { 
        var offset = jQuery('#tacco1').position();
        jQuery('.mk-moving-image-shortcode').css('left',offset.left+70);
    }
...
/* Posizionare immagine in home */
/* -------------------------------------------------------------------- */
function position_image() {
	
	if($('.home .vc_col-sm-6 img').length !=0){				
		var h=$('.wp-image-330').height()/2;
		$('.wp-image-330').css('margin-top','-'+h+'px');
 		/*
$('#tacco1').css('margin-top','-'+h+'px');
 		$('#t1').css('margin-top','-'+h+'px');
 		$('#t2').css('margin-top','-'+h+'px');
 		$('#t3').css('margin-top','-'+h+'px');
 		$('#t4').css('margin-top','-'+h+'px');
 		$('#baseT').css('margin-top','-'+h+'px');
*/
...
function filter_collection(selector){
    ...
    jQuery.ajax({
      ...
      url: ajaxurl,
      cache: false,
      data: {"action": "filter_portfolio", categoria:selector},
      success: function(response) {  
          
      		var part = response.split('<div id="stopqui">');  		 
	  	//	$('.mk-flexslider').html(part[0]);  
	  	 
	  		$('.mk-flexslider').remove();
	  	    $('#mk-filter-portfolio').after(part[0]);
...
function mk_sidebar_navigation() {
  ...
  $('.flex-viewport').each(function () {
    var $this = $(this),
      $mk_container = $this.find('.mk-flex-slides'),
    ...
    var mk_swiper = $slider_wrapper.swiper({
      ...
      onSwiperCreated: function(swiper) {
        ...
        onSlideChangeEnd: function() {
					...
					if (mk_swiper.activeLoopIndex==1) {
				     	$( '.mk-edge-next' ).fadeIn();
					 	$('.mk-edge-prev').fadeIn();
				    }
					if (mk_swiper.activeLoopIndex==2) {
				     	$( '.mk-edge-next' ).hide();
					 	$('.mk-edge-prev').fadeIn();
				    }
